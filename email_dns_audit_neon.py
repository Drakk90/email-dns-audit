...
"""
═══════════════════════════════════════════════════════════════════════════════
 EMAIL DNS AUDIT NEON v3.0 — Excel Unificado
...
def check_deps(auto=False):
    ...
    if auto:
        ...
        print("\033[92m[OK] Re-ejecuta el script.\033[0m"); sys.exit(0)
    print(msg); return False

if "--install-deps" in sys.argv: check_deps(auto=True)
...
COMMON_SELECTORS = ["selector1","selector2","google","k1","k2","k3","s1","s2",
    "dkim","mail","default","smtpapi","mandrill","fd","fd2","pic","pm","mxvault",
    "ses-1","ses-2","zoho","dynect","hubspotemail1","hubspotemail2",
    "everlytickey1","everlytickey2","sm","proton"]
...
def map_dkim_service(s):
    m = {"selector1":"Microsoft 365","selector2":"Microsoft 365","google":"Google Workspace",
         ...
         "ses-1":"Amazon SES","ses-2":"Amazon SES","zoho":"Zoho Mail","dynect":"Dyn",
         "hubspotemail1":"HubSpot","hubspotemail2":"HubSpot","everlytickey1":"Everlytic",
         "everlytickey2":"Everlytic","sm":"SparkPost","proton":"Proton Mail",
         "dkim":"Generico / Por validar","mail":"Generico / Por validar",
         "default":"Generico / Por validar","s1":"Generico / Por validar","s2":"Generico / Por validar"}
    return m.get(s, "Por validar (selector custom)")

def map_provider_purpose(p):
    if p in ("Microsoft 365","Google Workspace","Zoho Mail","Proton Mail"): return "Interno (correo corporativo)"
    if p in ("Mailchimp","HubSpot","Mailjet","Everlytic"): return "Marketing"
    if p in ("SendGrid","Mailgun","Amazon SES","Postmark","SparkPost","Mandrill",
             "Mandrill (Mailchimp transaccional)","Mailchimp / SendGrid"): return "Transaccional"
    return "Por validar"

def map_provider_mechanism(p):
    if p == "Microsoft 365": return "Conector M365"
    if p == "Google Workspace": return "Conector Google"
    if p in ("Zoho Mail","Proton Mail"): return "SMTP / Webmail"
    if p in ("SendGrid","Mailgun","Amazon SES","Postmark","SparkPost","Mandrill",
             "Mandrill (Mailchimp transaccional)","Mailjet","Mailchimp / SendGrid"): return "API / SMTP relay"
    if p in ("Mailchimp","HubSpot","Everlytic"): return "Plataforma SaaS"
    return "Por validar"
...
async def q(resolver_obj, name, rdtype):
    try: return await resolver_obj.resolve(name, rdtype, lifetime=6)
...
async def check_ns_soa(domain, ro, outdir, ip):
    ...
    soa_serial = "N/D"
    if soa:
        try: soa_serial = str(soa[0].serial)
        except: pass
...
async def check_dnssec(domain, ro, outdir, ip):
    dk = await q(ro, domain, "DNSKEY")
    ds_a = await q(ro, domain, "DS")
    dk_pub = "si" if dk else "no"
    ds_pub = "si" if ds_a else "no"
    ad = "no"; status = "N/D"; algos = []
    try:
        ro.use_edns(0, dns.flags.DO, 4096)
        ans = await ro.resolve(domain, "A", lifetime=6)
        if ans.response.flags & dns.flags.AD: ad = "si"
        status = dns.rcode.to_text(ans.response.rcode())
    except dns.resolver.NoAnswer: status = "NOERROR"
    except dns.resolver.NXDOMAIN: status = "NXDOMAIN"
    except Exception as e:
        if "SERVFAIL" in str(e).upper(): status = "SERVFAIL"
    if dk:
        for r in dk:
            try: algos.append(str(r.algorithm))
            except: pass
    algos_s = ",".join(sorted(set(algos))) if algos else "N/A"
    if status == "SERVFAIL": diag = "Bogus (cadena rota)"
    elif dk_pub=="si" and ds_pub=="si" and ad=="si": diag = "Secure"
    elif dk_pub=="si" and ds_pub=="no": diag = "Incompleto (DS no publicado)"
    elif dk_pub=="no" and ds_pub=="no": diag = "No implementado"
    else: diag = "Inconsistente"
...
async def check_spf(domain, ro, outdir, ip):
    ...
    if ans:
        for r in ans:
            try: txts.append(b"".join(r.strings).decode("utf-8",errors="replace"))
            except: pass
...
async def check_dkim(domain, sels, ro, outdir, ip, counter):
    found = []
    for sel in sels:
        ans = await q(ro, f"{sel}._domainkey.{domain}", "TXT")
        if not ans: continue
        try: rec = "".join(b"".join(r.strings).decode("utf-8",errors="replace") for r in ans)
        except: continue
        if "v=DKIM1" not in rec and "p=" not in rec: continue
...
async def check_dmarc(domain, ro, outdir, ip):
    ...
    r = {"pub":"no","record":"","p":"","sp":"","pct":"","rua":"","ruf":"",
         "aspf":"","adkim":"","fo":"","rf":"","ri":""}
    if not rec or "v=DMARC1" not in rec: return r
    r["pub"]="si"; r["record"]=rec
    def g(p):
        m = re.search(p, rec)
        return m.group(1) if m else ""
    r["p"]=g(r"p=([a-z]+)"); r["sp"]=g(r"sp=([a-z]+)")
    r["pct"]=g(r"pct=(\d+)") or "100"
    r["rua"]=g(r"rua=([^;]+)"); r["ruf"]=g(r"ruf=([^;]+)")
...
async def check_tls_rpt(domain, ro, outdir, ip):
    ...
    r = {"pub":"no","record":"","rua":""}
    if "v=TLSRPTv1" not in txt: return r
...
async def check_bimi(domain, ro, http, outdir, ip):
    ...
    if "v=BIMI1" not in txt: return r
    r["pub"]="si"; r["record"]=txt
    ...
    if ma:
        ...
        try:
            ...
            if resp.status_code == 200:
                pem = resp.content
                try: cert = x509.load_pem_x509_certificate(pem, default_backend())
                except: cert = x509.load_der_x509_certificate(pem, default_backend())
                r["vmc_status"]="Si"
                issuer_cn = ""
                for attr in cert.issuer:
                    if attr.oid._name == "commonName":
                        issuer_cn = attr.value; break
                r["vmc_issuer"] = issuer_cn or cert.issuer.rfc4514_string()
                try: r["vmc_exp"] = cert.not_valid_after_utc.isoformat()
                except: r["vmc_exp"] = cert.not_valid_after.isoformat()
...
async def audit_domain(domain, args, ro, http, outdir, counters, data, hallazgos):
    ...
    if args.selectors: sels.extend(args.selectors.split())
    info["dkim"] = await check_dkim(domain, sels, ro, outdir, args.resolver, counters["dkim_row"])
...
def process_results(domain, info, counters, data, hallazgos):
    ...
    if not info["dkim"]: hallazgos.append([f"H-{len(hallazgos)+1:03d}",domain,"DKIM","Sin DKIM","Alta","Configurar"])
    ...
    for prov in provs:
        ...
        for dk in info["dkim"]:
            if fw.lower() in dk["service"].lower(): firma="Si"; break
...
def build_excel(outdir, data, hallazgos, stats, args):
    ...
    def style_hdr(ws, row, n, fill=FILL_H1, font=F_H1):
    ...
    def fill_rows(ws, start, end, n):
        for r in range(start, end+1):
            f = FILL_BAND if (r-start)%2==1 else FILL_WHITE
            for c in range(1, n+1):
                cell = ws.cell(row=r, column=c)
                cell.fill = f; cell.font = F_BODY
    ...
    portada = [
        ...
        ("Marco de referencia","ISO/IEC 27001:2022 A.5.14, A.8.20, A.8.21, A.8.23 - NIST CSF 2.0 (PR.DS, PR.AA) - NIST SP 800-177 Rev.1 - M3AAWG Email Authentication BCP - RFC 7208 (SPF), 6376 (DKIM), 7489 (DMARC), 8460 (TLS-RPT), 8461 (MTA-STS)"),
    ...
    def add_sheet(name, headers, rows, status_col=None, sev_col=None, col_widths=None):
        ...
        if col_widths: widths(ws, col_widths)
        ws.freeze_panes = "A2"
    ...
    add_sheet("DKIM",
        ...
        col_widths=[5,24,16,22,50,14,14,14,16,16,16,22,14])
    ...
    # 6. DNSSEC (extra, alineado al Inventario)
    add_sheet("DNSSEC",
    ...
    for i,h in enumerate(mta_hdr, start=1): ws.cell(row=2,column=i,value=h)
    style_hdr(ws, 2, len(mta_hdr), fill=FILL_H2, font=F_H2)
    ...
    for i,h in enumerate(tls_hdr, start=1): ws.cell(row=tls_start+1,column=i,value=h)
    style_hdr(ws, tls_start+1, len(tls_hdr), fill=FILL_H2, font=F_H2)
    ...
    for i,h in enumerate(bimi_hdr, start=1): ws.cell(row=bimi_start+1,column=i,value=h)
    style_hdr(ws, bimi_start+1, len(bimi_hdr), fill=FILL_H2, font=F_H2)
    ...
    widths(ws, [24,30,28,18,28,18,16,30])

    # 8. Remitentes_Autorizados
    ...
    add_sheet("Hallazgos",
        ...
        hallazgos, sev_col="E",
        col_widths=[14,24,14,50,14,40])

    # 10. Resumen_Consolidado
    ...
    for i,h in enumerate(hdr, start=1): ws.cell(row=1,column=i,value=h)
    style_hdr(ws, 1, len(hdr))
        ...
        for dom_dir in sorted(ev_dir.iterdir()):
            if dom_dir.is_dir():
                for f in sorted(dom_dir.iterdir()):
                    ws.cell(row=r,column=1,value=dom_dir.name)
                    ws.cell(row=r,column=2,value=f.name)
                    ws.cell(row=r,column=3,value=str(f.relative_to(outdir)))
                    for c in range(1,4):
                        cell = ws.cell(row=r,column=c)
                        cell.font=F_BODY; cell.alignment=AL; cell.border=BORDER
                        cell.fill = FILL_BAND if r%2==0 else FILL_WHITE
                    r += 1
    widths(ws, [24,30,50])
    ws.freeze_panes = "A2"
...
def banner():
    art = (f"[bold {NM}]╔══════════════════════════════════════════════════════════════╗[/]\n"
           f"[bold {NM}]║[/]  [bold {NC}]E M A I L[/]  [bold {NG}]D N S[/]  [bold {NO}]A U D I T[/]  [bold {NY}]v 3 . 0[/]            [bold {NM}]║[/]\n"
           f"[bold {NM}]║[/]  [{NP} italic]Excel Unificado         [/]      [bold {NM}]║[/]\n"
           f"[bold {NM}]╚══════════════════════════════════════════════════════════════╝[/]\n"
...
def render_panel(info):
    ...
    if s["pub"]=="si":
        ...
        else: t.add_row("SPF", cell(f"{s['all']} · lookups={s['lookups']}", "warn"))
    else: t.add_row("SPF", cell("No publicado", "fail"))
    if info["dkim"]:
        sels = ", ".join([f"{r['selector']}({r['bits']})" for r in info["dkim"][:3]])
        if len(info["dkim"])>3: sels += f" +{len(info['dkim'])-3}"
        t.add_row("DKIM", cell(sels, "ok"))
    else: t.add_row("DKIM", cell("Sin selectores", "fail"))
    if dm["pub"]=="si":
        lvl = "ok" if dm["p"]=="reject" else "warn" if dm["p"]=="quarantine" else "fail"
        t.add_row("DMARC", cell(f"p={dm['p']} pct={dm['pct']}", lvl))
    else: t.add_row("DMARC", cell("No publicado", "fail"))
    diag = dns_d["diag"]
    lvl = "ok" if diag=="Secure" else "warn" if diag.startswith("Incompleto") else "fail"
    t.add_row("DNSSEC", cell(diag, lvl))
    if mt["mode"]=="enforce": t.add_row("MTA-STS", cell(f"enforce", "ok"))
    elif mt["mode"]=="testing": t.add_row("MTA-STS", cell("testing", "warn"))
    else: t.add_row("MTA-STS", cell("No publicado", "fail"))
    ...
    else: t.add_row("BIMI", cell("No publicado", "fail"))
...
async def run_audit(args):
    ...
    if not domains:
        console.print(f"[{NR}]ERROR: Sin dominios.[/]"); return
    ...
    for i, domain in enumerate(domains, 1):
        try:
            ...
            console.print(f"[{NR}]ERROR {domain}: {e}[/]"); continue
    for h in hallazgos:
        sev = h[4]
...
def parse_args():
    p = argparse.ArgumentParser(description="Email DNS Audit v3.0 - Excel Unificado",
                                  formatter_class=argparse.RawTextHelpFormatter)
    p.add_argument("--domains","-d", help="Archivo con dominios")
    p.add_argument("--selectors","-s", default="", help="Selectores DKIM extra")
    p.add_argument("--resolver","-r", default="1.1.1.1", help="Resolver DNS")
...
def main():
    args = parse_args()
    if not args.domains:
        console.print(f"[{NR}]ERROR: --domains requerido[/]"); sys.exit(1)
    if not Path(args.domains).is_file():
        console.print(f"[{NR}]ERROR: No existe {args.domains}[/]"); sys.exit(1)
    try: asyncio.run(run_audit(args))
    except KeyboardInterrupt:
        console.print(f"\n[{NR}]Interrumpido.[/]"); sys.exit(130)
